...
def reactivate_device(
    ...
    """Alias for restore_device.

    Historical naming: users asked for "deactivate/reactivate" semantics; internally
    devices are soft-deleted (DeviceStatus.deleted) and restored.
    """

    return restore_device(request=request, device_id=device_id, db=db, admin_actor=admin_actor)
