...
<#[
    ...
    This script exercises the full admin/device contract end-to-end:
      1) Looks up a device by DeviceKey via /api/v1/admin/devices.
      2) Upserts a policy document via /api/v1/admin/policies.
      3) Assigns that policy in enforce mode via /api/v1/admin/assign-policy.
      4) Prints device health before and after you run the agent locally.
      5) Downloads run details /api/v1/admin/runs/{id} and surfaces diagnostics:
           - failures/non-ok items
           - run items overview
           - winget-only evidence table
      6) Asserts per-phase expectations (present/absent + version pin/min).
