...
try:
    ...
    logger.warning(
        "hook-cryptography: failed to determine whether cryptography is using OpenSSL >= 3.0.0",
        exc_info=True,
    )
...
if uses_openssl3:
    ...
    if is_module_satisfies("PyInstaller >= 6.0"):
        ...
        openssl_lib = [
            ...
            lib_fullpath
            for lib_name, lib_fullpath in linked_libs
            if os.path.basename(lib_name) == SSL_LIB_NAME
        ]
    ...
    if openssl_lib:
        logger.info(
            "hook-cryptography: cryptography uses dynamically-linked OpenSSL: %r", openssl_lib
        )
        ...
        if ossl_modules_dir.is_dir():
            logger.debug(
                "hook-cryptography: collecting OpenSSL modules directory: %r", str(ossl_modules_dir)
            )
