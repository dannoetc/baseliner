...
def _collect_graphviz_files():
    ...
    if not dot_binary:
        logger.warning("hook-pygraphviz: 'dot' program not found in PATH!")
    ...
    for program_name in progs:
        ...
        logger.debug(
            "hook-pygraphviz: collecting graphviz program %r: %r", program_name, program_binary
        )
    ...
    if compat.is_win:
        ...
        if hasattr(bindepend, "get_imports"):
            # PyInstaller >= 6.0
            dot_imports = [
                path for name, path in bindepend.get_imports(dot_binary) if path is not None
            ]
        else:
            # PyInstaller < 6.0
        ...
        if not graphviz_lib_paths:
            logger.warning(
                "hook-pygraphviz: could not determine location of graphviz shared libraries!"
            )
            return binaries, datas

        graphviz_lib_dir = pathlib.Path(graphviz_lib_paths[0]).parent
        logger.debug(
            "hook-pygraphviz: location of graphviz shared libraries: %r", str(graphviz_lib_dir)
        )
    ...
    datas += [
        (str(file), plugin_dest_dir) for file in plugin_dir.glob("config*")
    ]  # e.g., `config6`
