...
hiddenimports += hookutils.collect_submodules(
        ...
        if extension_files:
            if len(extension_files) > 1:
                hookutils.logger.warning(
                    "Found multiple cv2 extension candidates: %s", extension_files
                )
...
config_file = [
    ...
    in (f"config-{sys.version_info[0]}.{sys.version_info[1]}.py", "config-3.py")
]
...
if config_file:
    try:
        ...
        hookutils.logger.warning(
            "Failed to determine location of cv2 extension module!", exc_info=True
        )
