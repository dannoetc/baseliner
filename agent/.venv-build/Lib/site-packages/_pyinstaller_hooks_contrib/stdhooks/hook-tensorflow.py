...
if is_linux and dist is not None:
    ...
    def _infer_nvidia_hiddenimports():
        ...
        env = {"extra": "and-cuda"}
        requirements = [
            req.name for req in requirements if req.marker is None or req.marker.evaluate(env)
        ]
    ...
    try:
        ...
        logger.warning(
            "hook-tensorflow: failed to infer NVIDIA CUDA hidden imports!", exc_info=True
        )
        nvidia_hiddenimports = []
    logger.info(
        "hook-tensorflow: inferred hidden imports for CUDA libraries: %r", nvidia_hiddenimports
    )
