...
if is_module_satisfies("PyInstaller >= 6.0"):
    ...
    if compat.is_linux:
        ...
        logger.info(
            "hook-torch: inferred hidden imports for CUDA libraries: %r", nvidia_hiddenimports
        )
        ...
        def _collect_mkl_dlls():
            ...
            if conda_torch_dist:
                ...
                if "mkl" not in conda_torch_dist.dependencies:
                    logger.info(
                        "hook-torch: this torch build (Anaconda package) does not depend on MKL..."
                    )
                    return []

                logger.info(
                    "hook-torch: collecting DLLs from MKL and its dependencies (Anaconda packages)"
                )
                ...
                mkl_binaries = []
                logger.info(
                    "hook-torch: collecting DLLs from MKL and its dependencies: %r", requirements
                )
