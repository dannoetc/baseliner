...
class TestWin32API(unittest.TestCase):
    ...
    def test_enum_resource_names(self):
        with self.load_library(win32api, "shell32.dll") as handle:
            ...
            for resource_type in resource_types:
                ...
                resource_names = self.module.EnumResourceNames(handle, resource_type)
                self.assertEqual(resource_names, expected)
                # check that the #<index> format works
                resource_names = self.module.EnumResourceNames(handle, self._id2str(resource_type))
                self.assertEqual(resource_names, expected)
        ...
        with self.load_library(win32api, "shell32.dll") as handle:
            ...
            for resource_type in resource_types:
                ...
                for resource_name in resource_names:
                    expected = win32api.EnumResourceLanguages(handle, resource_type, resource_name)
                    ...
                    # check that the #<index> format works
                    resource_languages = self.module.EnumResourceLanguages(
                        handle, self._id2str(resource_type), self._id2str(resource_name)
                    )
                    self.assertEqual(resource_languages, expected)
    ...
    def test_load_resource(self):
        ...
        with self.assertRaises(error):
            with self.nofaulthandler():
                self.module.LoadResource(handle, resource_type, resource_name, 12435)
    ...
    def test_update_resource(self):
        ...
        with self.load_library(self.module, filename) as handle:
            ...
            resource_language = module.EnumResourceLanguages(handle, resource_type, resource_name)[
                -1
            ]
            resource = module.LoadResource(handle, resource_type, resource_name, resource_language)

        # when
        with self.resource_update(self.module, filename) as handle:
            module.UpdateResource(
                handle, resource_type, resource_name, resource[:-2], resource_language
            )

        # then
        with self.load_library(self.module, filename) as handle:
            updated = module.LoadResource(handle, resource_type, resource_name, resource_language)
    ...
    def test_update_resource_with_unicode(self):
        ...
        with self.resource_update(module, filename) as handle:
            with self.assertRaises(TypeError):
                module.UpdateResource(
                    handle, resource_type, resource_name, resource, resource_language
                )
