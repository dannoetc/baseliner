...
from win32ctypes.core import _authentication, _backend, _common
...
def CredEnumerate(Filter=None, Flags=0):
    ...
    with _pywin32error():
        if _backend == "cffi":
            ...
            count = pcount[0]
            data = _common.dereference(_common.ffi.cast(f"PCREDENTIAL*[{count}]", pppcredential))
            ...
            _authentication._CredEnumerate(Filter, Flags, _common.byreference(count), pppcredential)
            count = count.value
            data = _common.dereference(
                _common.cast(ppcredential, _common.POINTER(_authentication.PCREDENTIAL * count))
            )
            memory = pcredential
    try:
        result = []
