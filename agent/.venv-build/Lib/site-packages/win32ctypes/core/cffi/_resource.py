...
def ENUMRESLANGPROC(callback):
    def wrapped(hModule, lpszType, lpszName, wIDLanguage, lParam):
        return callback(hModule, resource(lpszType), resource(lpszName), wIDLanguage, lParam)
...
def _EnumResourceNames(hModule, lpszType, lpEnumFunc, lParam):
    ...
    check_false(
        dlls.kernel32.EnumResourceNamesW(PVOID(hModule), RESOURCE(lpszType), callback, lParam),
...
def _SizeofResource(hModule, hResInfo):
    return check_zero(
        dlls.kernel32.SizeofResource(PVOID(hModule), hResInfo), function_name="SizeofResource"
    )


def _LoadResource(hModule, hResInfo):
    return check_null(
        dlls.kernel32.LoadResource(PVOID(hModule), hResInfo), function_name="LoadResource"
    )
...
def _BeginUpdateResource(pFileName, bDeleteExistingResources):
    result = check_null(
        dlls.kernel32.BeginUpdateResourceW(str(pFileName), bDeleteExistingResources)
    )
...
def _UpdateResource(hUpdate, lpType, lpName, wLanguage, cData, cbData):
    ...
    check_false(
        dlls.kernel32.UpdateResourceW(
            PVOID(hUpdate), RESOURCE(lpType), RESOURCE(lpName), wLanguage, PVOID(lpData), cbData
        ),
