...
#
import ctypes
from ctypes import POINTER, Structure, c_char_p, c_void_p, c_wchar_p, cast
from ctypes.wintypes import BOOL, DWORD, FILETIME, LPCWSTR
...
from ._util import check_false_factory, dlls, function_factory
        ...
        for key in SUPPORTED_CREDKEYS:
            if key in credential:
                if key != "CredentialBlob":
                    ...
                    c_creds.CredentialBlobSize = ctypes.sizeof(blob_data) - ctypes.sizeof(
                        ctypes.c_wchar
                    )
    ...
    for key in SUPPORTED_CREDKEYS:
        if key != "CredentialBlob":
            ...
            blob = _PyBytes_FromStringAndSize(
                cast(creds.CredentialBlob, c_char_p), creds.CredentialBlobSize
            )
...
_CredWrite = function_factory(
    dlls.advapi32.CredWriteW, [PCREDENTIAL, DWORD], BOOL, check_false_factory("CredWrite")
)
...
_CredDelete = function_factory(
    dlls.advapi32.CredDeleteW, [LPCWSTR, DWORD, DWORD], BOOL, check_false_factory("CredDelete")
)
