...
class UnicodeStringWrapperPostProcessor:
    ...
    def render_pascal_16(self):
        try:
            ...
            self.pe.get_warnings().append(
                "Failed rendering pascal string, " "attempting to read from RVA 0x{0:x}".format(
                    self.rva_ptr + 2
                )
            )
    ...
    def render_unicode_16(self):
        try:
            ...
            self.pe.get_warnings().append(
                "Failed rendering unicode string, " "attempting to read from RVA 0x{0:x}".format(
                    self.rva_ptr
                )
            )
            ...
            for key in keys:
                ...
                if isinstance(val, (int, long)):
                    ...
                    if key.startswith("Signature"):
                        ...
                        val_str = "".join(
                            [
                                chr(i) if (i in printable_bytes) else "\\x{0:02x}".format(i)
...
class UnwindInfo(StructureWithBitfields):
    ...
    * optional ExceptionHandler and FunctionEntry fields
    ...
    def dump(self, indentation=0):
        ...
        dump.append(
            "Unwind codes: " + "; ".join([str(c) for c in self.UnwindCodes if c.is_valid()])
        )
        return dump
    ...
    def __pack__(self):
        ...
        if self._opt_field_name != None:
            data[self._full_size - STRUCT_SIZEOF_TYPES["I"] : self._full_size] = struct.pack(
                "<I", getattr(self, self._opt_field_name)
            )
...
class PrologEpilogOpAllocLarge(PrologEpilogOp):
    ...
    def get_alloc_size(self):
        return (
            self.struct.AllocSizeInQwords * 8 if self.struct.OpInfo == 0 else self.struct.AllocSize
        )
...
class PE:
    ...
    def __parse__(self, fname, data, fast_load):
        ...
        if (
            ...
            and len(self.__data__[optional_header_offset : optional_header_offset + 0x200])
        ...
        if self.PE_TYPE is None:
            self.__warnings.append(
                "Invalid type 0x{0:04x} in Optional Header.".format(self.OPTIONAL_HEADER.Magic)
            )
        ...
        if self.get_section_by_rva(self.OPTIONAL_HEADER.AddressOfEntryPoint) is not None:
            ...
            if ep_offset > len(self.__data__):
                self.__warnings.append(
                    ...
                    " file. AddressOfEntryPoint: 0x%x" % self.OPTIONAL_HEADER.AddressOfEntryPoint
                )
                            ...
                            for st_entry in entry.StringTable:
                                for key, entry in list(st_entry.entries.items()):
                                    ...
                                    if len(entry) > lengths[1]:
                                        ...
                                        file_data[offsets[1] : offsets[1] + len(encoded_data)] = (
                                            encoded_data
                                        )
    ...
    def parse_sections(self, offset):
        ...
        for i in range(self.FILE_HEADER.NumberOfSections):
            ...
            if self.adjust_PointerToRawData(section.PointerToRawData) > len(self.__data__):
                simultaneous_errors += 1
                self.__warnings.append(
            ...
            if section.__dict__.get("IMAGE_SCN_MEM_WRITE", False) and section.__dict__.get(
                "IMAGE_SCN_MEM_EXECUTE", False
            ):
        ...
        while True:
            ...
            if section is None:
                ...
                sections_after_offset = [
                    s.PointerToRawData for s in self.sections if s.PointerToRawData > file_offset
                ]
            ...
            bound_imports.append(
                BoundImportDescData(struct=bnd_descr, name=name_str, entries=forwarder_refs)
            )
    ...
    def parse_directory_load_config(self, rva, size):
        ...
        if self.PE_TYPE == OPTIONAL_HEADER_MAGIC_PE:
            ...
            self.__warnings.append(
                "Don't know how to parse LOAD_CONFIG information for non-PE32/" "PE32+ file"
            )
            return None
    ...
    def parse_function_override_data(self, rva):
        ...
        if not bdd_info:
            self.__warnings.append("Invalid bdd info. Can't read " "data at RVA: 0x%x" % rva)
            return func_relocs, bdd_relocs
        rva += Structure(format).sizeof()
    ...
    def parse_image_base_relocation_list(self, rva, size, fmt=None):
        ...
        while rva < end:
            ...
            if rlc.SizeOfBlock > self.OPTIONAL_HEADER.SizeOfImage:
                self.__warnings.append(
                    "Invalid relocation information. SizeOfBlock too large" ": %d" % rlc.SizeOfBlock
                )
                break
    ...
    def parse_relocations(self, data_rva, rva, size):
        ...
        for idx in range(len(data) // 2):
            ...
            if (reloc_offset, reloc_type) in offsets_and_type:
                self.__warnings.append(
                    "Overlapping offsets in relocation data " "at RVA: 0x%x" % (reloc_offset + rva)
                )
                break
            ...
            entries.append(
                RelocationData(struct=entry, type=reloc_type, base_rva=rva, rva=reloc_offset + rva)
            )
    ...
    def parse_relocations_with_format(self, data_rva, rva, size, format):
        ...
        for idx in range(len(data) // entry_size):
            ...
            entries.append(RelocationData(struct=entry, base_rva=rva, rva=reloc_offset + rva))
    ...
    def parse_resources_directory(self, rva, size=0, base_rva=None, level=0, dirs=None):
        ...
        try:
            ...
            self.__warnings.append(
                "Invalid resources directory. Can't read " "directory data at RVA: 0x%x" % rva
            )
            return None
        ...
        if resource_dir is None:
            ...
            # still have a valid PE file
            self.__warnings.append(
                "Invalid resources directory. Can't parse " "directory data at RVA: 0x%x" % rva
            )
            return None
        ...
        if number_of_entries > MAX_ALLOWED_ENTRIES:
            self.__warnings.append(
                "Error parsing the resources directory. "
                "The directory contains %d entries (>%s)" % (number_of_entries, MAX_ALLOWED_ENTRIES)
            )
            return None
    ...
    def parse_resource_data_entry(self, rva):
        ...
        try:
            ...
            self.__warnings.append(
                "Error parsing a resource directory data entry, " "the RVA is invalid: 0x%x" % (rva)
            )
            return None
    ...
    def parse_version_information(self, version_struct):
        ...
        while True:
            ...
            ustr_offset = (
                version_struct.OffsetToData + stringfileinfo_offset + versioninfo_struct.sizeof()
            )
            try:
                ...
                if stringfileinfo_struct.Type in (0, 1) and stringfileinfo_struct.ValueLength == 0:
                    ...
                    while True:
                        ...
                        while entry_offset < stringtable_offset + stringtable_struct.Length:
                            ...
                            ustr_offset = (
                                version_struct.OffsetToData + entry_offset + string_struct.sizeof()
                            )
                            try:
    ...
    def parse_export_directory(self, rva, size, forwarded_only=False):
        ...
        if section:
            safety_boundary = (
                section.VirtualAddress + len(section.get_data()) - export_dir.AddressOfNames
            )
        ...
        if section:
            safety_boundary = (
                section.VirtualAddress + len(section.get_data()) - export_dir.AddressOfFunctions
            )
    ...
    def dump_info(self, dump=None, encoding="ascii"):
        ...
        if hasattr(self, "DIRECTORY_ENTRY_BOUND_IMPORT"):
            ...
            for bound_imp_desc in self.DIRECTORY_ENTRY_BOUND_IMPORT:
                ...
                dump.add_line(
                    "DLL: {0}".format(bound_imp_desc.name.decode(encoding, "backslashreplace_"))
                )
                dump.add_newline()
                ...
                for bound_imp_ref in bound_imp_desc.entries:
                    ...
                    dump.add_line(
                        "DLL: {0}".format(bound_imp_ref.name.decode(encoding, "backslashreplace_")),
                        4,
                    )
                    dump.add_newline()
                    ...
                    while entry_idx < len(reloc.entries):
                        ...
                        if entry.type == RELOCATION_TYPE["IMAGE_REL_BASED_ABSOLUTE"]:
                            ...
                            self.set_word_at_rva(
                                entry.rva,
                                (self.get_word_at_rva(entry.rva) + relocation_difference >> 16)
                                & 0xFFFF,
                            )
