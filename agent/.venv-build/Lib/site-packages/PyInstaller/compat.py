...
is_musl = (
    is_linux and "musl" in subprocess.run(["ldd"], capture_output=True, encoding="utf-8").stderr
)
...
if is_win:
    if _setup_py_mode:
        ...
        try:
            ...
            from win32ctypes.pywin32 import (
                pywintypes,  # noqa: F401, E402
                win32api,  # noqa: F401, E402
            )
        except ImportError as e:
            raise SystemExit(
...
if is_win:
    ...
    machine = _pyi_machine(
        os.environ.get("PROCESSOR_ARCHITECTURE", platform.machine()), platform.system()
    )
else:
...
def check_requirements():
    ...
    if sys.implementation.name != "cpython":
        raise SystemExit(
            f"ERROR: PyInstaller does not support {sys.implementation.name}. Only CPython is supported."
        )
