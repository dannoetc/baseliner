...
class BUNDLE(Target):
    ...
    def _compute_relative_crosslink(crosslink_location, crosslink_target):
        ...
        return os.path.join(
            *[
                ".."
                for level in pathlib.PurePath(crosslink_location)
                .relative_to("Contents")
                .parent.parts
            ],
    ...
    def _process_bundle_toc(self, toc):
        ...
        for dest_name, src_name, typecode in toc:
            ...
            for parent_dir in parent_dirs:
                ...
                directory_type = (
                    _BINARY_DIR_TYPE if typecode == "BINARY" else _DATA_DIR_TYPE
                )  # default
        ...
        for directory_path, directory_type in directory_types.items():
            ...
            requires_crosslink = (
                parent_dir == _TOP_LEVEL_DIR or directory_types.get(parent_dir) == _MIXED_DIR_TYPE
            )
        ...
        for orig_dest_name, src_name, typecode in toc:
            ...
            if crosslink_base_dir is not None:
                ...
                requires_crosslink = (
                    parent_dir == _TOP_LEVEL_DIR
                    or directory_types.get(parent_dir) == _MIXED_DIR_TYPE
                )
            ...
            if typecode == "SYMLINK" and requires_crosslink:
                ...
                bundle_toc.append(
                    (os.path.join(crosslink_base_dir, orig_dest_name), src_name, typecode)
                )
                continue
    ...
    def assemble(self):
        ...
        shutil.copyfile(
            self.icon, os.path.join(self.name, "Contents", "Resources", os.path.basename(self.icon))
        )
        ...
        for dest_name, src_name, typecode in bundle_toc:
            ...
            if typecode in ("EXTENSION", "BINARY"):
                orig_dest_name = str(
                    pathlib.PurePath(dest_name).relative_to(CONTENTS_FRAMEWORKS_PATH)
                )
            ...
            if typecode == "SYMLINK":
                ...
                assert (
                    typecode != "DEPENDENCY"
                ), "MERGE DEPENDENCY entries are not supported in BUNDLE!"
        ...
        try:
            osxutils.sign_binary(
                self.name, self.codesign_identity, self.entitlements_file, deep=True
            )
    ...
    def verify_bundle_signature(bundle_dir):
        ...
        try:
            ...
            logger.info(
                "xattr package not available; skipping verification of extended attributes!"
            )
            return
        ...
        for entry in pathlib.Path(bundle_dir).rglob("*"):
            ...
            if any([codesign_attr in file_attrs for codesign_attr in CODESIGN_ATTRS]):
                raise ValueError(
                    f"Code-sign attributes found in extended attributes of {str(entry)!r}!"
                )
