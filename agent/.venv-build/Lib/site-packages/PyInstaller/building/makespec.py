...
class SourceDestAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, value, option_string=None):
        try:
            ...
            (separator,) = (
                m for m in re.finditer(rf"(^\w:[/\\])|[:{os.pathsep}]", value) if not m[1]
            )
        except ValueError:
            # Split into SRC and DEST failed, wrong syntax
            raise argparse.ArgumentError(
                self, f"Wrong syntax, should be {self.option_strings[0]}=SOURCE:DEST"
            )
...
def main(
    ...
    **_kwargs,
    ...
    if _kwargs["shorthand_manifest"]:
        ...
        logger.log(
            logging.DEPRECATION,
            "PyInstaller v7 will remove the -m shorthand flag. Please use --manifest=%s instead",
            manifest,
        )
    if manifest:
        if "<" in manifest:
