...
from PyInstaller import DEFAULT_DISTPATH, DEFAULT_WORKPATH, HOMEPATH, compat, isolated
...
from PyInstaller.building.utils import (
    ...
    destination_name_for_extension,
    format_binaries_and_datas,
    postprocess_binaries_toc_pywin32,
    postprocess_binaries_toc_pywin32_anaconda,
)
from PyInstaller.compat import is_conda, is_darwin, is_linux, is_win
from PyInstaller.depend import bindepend
from PyInstaller.depend.analysis import HOOK_PRIORITY_USER_HOOKS, initialize_modgraph
...
def discover_hook_directories():
    ...
    entry_points = sorted(
        entry_points, key=lambda x: x.module.startswith("_pyinstaller_hooks_contrib")
    )
    ...
    for entry_point in entry_points:
        ...
        try:
            ...
            logger.warning(
                "discover_hook_directories: Failed to process hook entry point '%s': %s",
                entry_point,
                msg,
            )
            continue
        ...
        location_priority = (
            HOOK_PRIORITY_CONTRIBUTED_HOOKS
            if entry_point.module.startswith("_pyinstaller_hooks_contrib")
            else HOOK_PRIORITY_UPSTREAM_HOOKS
        )

        # Append entries
        hook_directories.extend(
            [
                (hook_directory_entry, location_priority)
...
class Analysis(Target):
    ...
    def __init__(
        ...
        **_kwargs,
        ...
        if site_packages_pathex:
            logger.log(
                logging.DEPRECATION,
                "Foreign Python environment's site-packages paths added to --paths/pathex:\n%s\n"
                ...
                "an error in PyInstaller 7.0.",
                pprint.pformat(site_packages_pathex),
            )
        ...
        if hookspath:
            self.hookspath.extend(
                [(os.path.expanduser(path), HOOK_PRIORITY_USER_HOOKS) for path in hookspath]
            )
    ...
    def assemble(self):
        ...
        for dest_name, src_name, typecode in combined_toc:
            ...
            if detected_typecode is not None:
                if detected_typecode != typecode:
                    logger.debug(
                        "Reclassifying collected file %r from %s to %s...",
                        src_name,
                        typecode,
                        detected_typecode,
                    )
            ...
            try:
                ...
                for dest_name, src_name, typecode in set(ctypes_binaries):
                    ...
                    if bindepend.classify_binary_vs_data(src_name) not in (None, "BINARY"):
                        logger.warning(
                            "Ignoring %s found via ctypes - not a valid binary!", src_name
                        )
                        continue
                    self.binaries.append((dest_name, src_name, typecode))
            except Exception as ex:
                raise RuntimeError(
                    f"Failed to scan the module '{name}'. This is a bug. Please report it."
                ) from ex
        ...
        self.scripts = normalize_toc(
            self.scripts
        )  # Should not really contain duplicates, but just in case...
        ...
        self.datas = compile_glib_schema_files(
            self.datas, os.path.join(CONF["workpath"], "_pyi_gschema_compilation")
        )
        self.datas = normalize_toc(self.datas)
        ...
        optim_level = (
            self.optimize
        )  # We could extend this with per-module settings, similar to `collect_mode`.
        for name, src_path, typecode in pure_pymodules_toc:
            assert typecode == "PYMODULE"
            collect_mode = _get_module_collection_mode(
                self.graph._module_collection_mode, name, self.noarchive
            )
        ...
        if base_modules_toc:
            ...
            self.datas.append(
                (os.path.basename(base_library_zip), base_library_zip, "DATA")
            )  # Bundle as data file.
        ...
        self.binaries.extend(
            find_binary_dependencies(
                self.binaries, collected_packages, self.graph._bindepend_symlink_suppression
            )
        )
        ...
        if is_darwin:
            ...
            for dest_name, src_name, typecode in self.binaries:
                try:
                    ...
                    logger.warning(
                        "Failed to query macOS SDK version of %r!", src_name, exc_info=True
                    )
            ...
            if binaries_with_invalid_sdk:
                logger.warning(
                    "Found one or more binaries with invalid or incompatible macOS SDK version:"
                )
                for dest_name, src_name, sdk_version in binaries_with_invalid_sdk:
                    logger.warning(
                        " * %r, collected as %r; version: %r", src_name, dest_name, sdk_version
                    )
                logger.warning(
                    "These binaries will likely cause issues with code-signing and hardened runtime!"
                )
    ...
    def exclude_system_libraries(self, list_of_exceptions=None):
        ...
        self.binaries = [
            ...
            if _should_include_system_binary(entry, list_of_exceptions or [])
        ]
