...
class Splash(Target):
    ...
    def __init__(self, image_file, binaries, datas, **kwargs):
        ...
        tcltk_libs = [
            (os.path.basename(src_name), src_name, "BINARY")
            for src_name in (self.tcl_lib, self.tk_lib)
        ]
        ...
        if not self.uses_tkinter:
            self.binaries.extend(
                entry for entry in tcltk_tree if entry[0] in self.splash_requirements
            )
    ...
    def assemble(self):
        ...
        def _resize_image(_image, _orig_size):
            if PILImage:
                ...
                logger.info(
                    "Resized image %s from dimensions %r to (%d, %d)",
                    self.image_file,
                    _orig_size,
                    _w,
                    _h,
    ...
    def _check_tcl_tk_compatibility():
        ...
        if tcl_version < (8, 6) or tk_version < (8, 6):
            logger.warning(
                ...
                "bootloader, which was tested against Tcl/Tk 8.6",
                *tcl_version,
                *tk_version,
            )
