...
class PYZ(Target):
    ...
    def __init__(self, *tocs, **kwargs):
        ...
        bootstrap_module_names = set(
            name for name, _, typecode in self.dependencies if typecode == "PYMODULE"
        )
        self.toc = []
        self.code_dict = {}
        for toc in tocs:
            ...
            for entry in toc:
                ...
                assert typecode in {
                    ...
                    "PYMODULE-1",
                    "PYMODULE-2",
                }, f"Invalid entry passed to PYZ: {entry}!"
...
class PKG(Target):
    ...
    def assemble(self):
        ...
        pkg_file = pathlib.Path(
            self.name
        ).resolve()  # Used to detect attempts at PKG feeding itself
        ...
        for dest_name, src_name, typecode in self.toc:
            ...
            if typecode not in {"OPTION", "DEPENDENCY", "SYMLINK"}:
                if not os.path.exists(src_name):
                    if strict_collect_mode:
                        raise ValueError(
                            f"Non-existent resource {src_name}, meant to be collected as {dest_name}!"
                        )
                    else:
                        logger.warning(
                            "Ignoring non-existent resource %s, meant to be collected as %s",
                            src_name,
                            dest_name,
                        )
                        continue
            ...
            if typecode in ("BINARY", "EXTENSION"):
                if self.exclude_binaries:
                    ...
                    archive_toc.append(
                        (
                            dest_name,
                            src_name,
                            self.cdict.get(typecode, False),
                            self.xformdict[typecode],
                        )
                    )
                ...
                if self.exclude_binaries:
                    ...
                    archive_toc.append(
                        (dest_name, src_name, self.cdict.get(typecode, False), carchive_typecode)
                    )
                ...
                bootstrap_toc.append(
                    (dest_name, src_name, self.cdict.get(typecode, False), self.xformdict[typecode])
                )
            elif typecode == "PYZ":
                ...
                archive_toc.append(
                    ("PYZ.pyz", src_name, self.cdict.get(typecode, False), self.xformdict[typecode])
                )
            else:
        ...
        logger.info(
            "Building PKG (CArchive) %s completed successfully.", os.path.basename(self.name)
        )
...
class EXE(Target):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.contents_directory in ("", "."):
            self.contents_directory = (
                None  # Re-enable old onedir layout without contents directory.
            )
        elif (
            self.contents_directory == ".."
            or "/" in self.contents_directory
            or "\\" in self.contents_directory
        ):
            raise SystemExit(
        ...
        if is_win:
            if not self.icon:
                ...
                self.icon = os.path.join(
                    os.path.dirname(os.path.dirname(__file__)), "bootloader", "images", ico
                )
            ...
            self.manifest = winmanifest.create_application_manifest(
                self.manifest, self.uac_admin, self.uac_uiaccess
            )
            ...
            if self.versrsrc:
                if isinstance(self.versrsrc, versioninfo.VSVersionInfo):
                    ...
                    raise TypeError(
                        f"Unsupported type for version info argument: {type(self.versrsrc)!r}"
                    )
    ...
    def _check_guts(self, data, last_build):
        if not os.path.exists(self.name):
            logger.info(
                "Rebuilding %s because %s missing", self.tocbasename, os.path.basename(self.name)
            )
            return True
    ...
    def assemble(self):
        ...
        if is_win:
            ...
            osxutils.binary_to_target_arch(
                build_name, self.target_arch, display_name="Bootloader EXE"
            )
        ...
        if is_linux:
            ...
            p = subprocess.run(
                cmd, stderr=subprocess.STDOUT, stdout=subprocess.PIPE, encoding="utf-8"
            )
            if p.returncode:
    ...
    def _copy_windows_resource(self, build_name, resource_spec):
        ...
        if is_pe_file:
            ...
            try:
                ...
                raise IOError(
                    f"Failed to embed data file {src_filename!r} as Windows resource"
                ) from e
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if attempt < max_attempts - 1:
                    ...
                    raise RuntimeError(
                        f"Execution of {func_name!r} failed - no more attempts left!"
                    ) from e
    ...
    def assemble(self):
        ...
        for dest_name, src_name, typecode in self.toc:
            ...
            if os.pardir in os.path.normpath(dest_name).split(os.sep) or os.path.isabs(dest_name):
                raise SystemExit(
                    "ERROR: attempting to store file outside of the dist directory: %r. Aborting."
                    % dest_name
                )
            ...
            if typecode == "SYMLINK":
                ...
                os.symlink(
                    src_name, dest_path
                )  # Create link at dest_path, pointing at (relative) src_name
        ...
        for entry in toc:
            ...
            if key not in self._dependencies:
                ...
                if dep_path.endswith(path_to_exe):
                    logger.debug(
                        "Ignoring self-reference of %r for %s, located in %s - duplicated TOC entry?",
                        key,
                        path_to_exe,
                        dep_path,
                    )
                    ...
                    continue
                logger.debug(
                    "Referencing %r to be a dependency for %s, located in %s",
                    key,
                    path_to_exe,
                    dep_path,
                )
