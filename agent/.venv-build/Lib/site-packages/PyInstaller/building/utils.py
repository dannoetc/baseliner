...
from PyInstaller.compat import is_aix, is_darwin, is_linux, is_win
...
def process_collected_binary(
    ...
    if use_upx:
        ...
        for upx_exclude_entry in upx_exclude:
            ...
            if src_path.match(upx_exclude_entry):
                logger.info(
                    "Disabling UPX for %s due to match in exclude pattern: %s",
                    src_name,
                    upx_exclude_entry,
                )
                use_upx = False
    ...
    if (use_upx or use_strip) and is_linux:
        ...
        if hmac_path.is_file():
            logger.info(
                "Disabling UPX and/or strip for %s due to accompanying .hmac file!", src_name
            )
            use_upx = use_strip = False
        elif chk_path.is_file():
            logger.info(
                "Disabling UPX and/or strip for %s due to accompanying .chk file!", src_name
            )
            use_upx = use_strip = False
    ...
    if is_darwin:
        try:
            ...
            target_rpath = str(
                pathlib.PurePath(
                    "@loader_path", *[".." for level in pathlib.PurePath(dest_name).parent.parts]
                )
            )
...
def _check_path_overlap(path):
    ...
    if CONF["workpath"].startswith(path):
        logger.error(
            'Specfile error: The output path "%s" contains WORKPATH (%s)', path, CONF["workpath"]
        )
        specerr += 1
    if CONF["specpath"].startswith(path):
        logger.error(
            'Specfile error: The output path "%s" contains SPECPATH (%s)', path, CONF["specpath"]
        )
        specerr += 1
    if specerr:
        raise SystemExit(
...
def _rmtree(path):
    ...
    if CONF["noconfirm"]:
        ...
        choice = input(
            'WARNING: The output directory "%s" and ALL ITS CONTENTS will be REMOVED! Continue? (y/N)'
            % path
        )
    else:
        raise SystemExit(
...
def format_binaries_and_datas(binaries_or_datas, workingdir=None):
    ...
    for src_root_path_or_glob, trg_root_dir in binaries_or_datas:
        ...
        if os.path.isabs(trg_root_dir):
            raise InvalidSrcDestTupleError(
                (src_root_path_or_glob, trg_root_dir), "DEST_DIR must be a relative path!"
            )
        ...
        if not src_root_paths:
            raise SystemExit(
                f"ERROR: Unable to find {src_root_path_or_glob!r} when adding binary and data files."
            )
        ...
        for src_root_path in src_root_paths:
            if os.path.isfile(src_root_path):
                ...
                toc_datas.add(
                    (
                        os.path.normpath(
                            os.path.join(trg_root_dir, os.path.basename(src_root_path))
                        ),
...
def replace_filename_in_code_object(code_object, filename):
    ...
    consts = tuple(
        replace_filename_in_code_object(const_co, filename)
        if isinstance(const_co, types.CodeType)
        else const_co
...
def _read_pyc_data(filename):
    ...
    if src_file.is_file():
        ...
        if pyc_data[:4] != compat.BYTECODE_MAGIC:
            raise ValueError(
                f"The .pyc module {filename} was compiled for incompatible version of python!"
            )
...
def postprocess_binaries_toc_pywin32(binaries):
    ...
    for dest_name, src_name, typecode in binaries:
        ...
        if (
            dest_path.parent == pathlib.PurePath(".")
            and src_path.parent.name.lower() in PYWIN32_SUBDIRS
        ):
