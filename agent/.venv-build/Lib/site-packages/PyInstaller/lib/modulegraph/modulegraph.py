...
class FlatPackage(BaseModule):
    def __init__(self, *args, **kwds):
        warnings.warn(
            "This class will be removed in a future version of modulegraph", DeprecationWarning
        )
...
class ModuleGraph(ObjectGraph):
    ...
    def scan_legacy_namespace_packages(self):
        ...
        self._legacy_ns_packages = {
            package_name: list(paths) for package_name, paths in legacy_ns_packages.items()
        }
    ...
    def import_hook(
        ...
        while target_module_partname:
            ...
            head, target_module_partname = (
                target_module_partname[:i],
                target_module_partname[i + 1 :],
            )
    ...
    def alias_module(self, src_module_name, trg_module_name):
        ...
        if trg_module is not None and not (
            isinstance(trg_module, AliasNode) and trg_module.identifier == src_module_name
        ):
            raise ValueError(
                'Target module "%s" already imported as "%s".' % (trg_module_name, trg_module)
            )
        ...
        if module is None:
            ...
            if co is not None:
                try:
                    ...
                    self.msg(
                        1,
                        "safe_import_module: SyntaxError in ",
                        pathname,
                    )
    ...
    def _load_module(self, fqname, pathname, loader):
        ...
        if loader is BUILTIN_MODULE:
            ...
            try:
                ...
                self.msg(
                    2,
                    "load_module: failed to obtain source for "
            ...
            if src is not None:
                ...
                try:
                    ...
                    self.msg(
                        2, "load_module: InvalidCompiledModule, " "Cannot load code", pathname, exc
                    )
    ...
    def _safe_import_hook(
        ...
        """
        self.msg(
            3, "_safe_import_hook", target_module_partname, source_module, target_attr_names, level
        )
        ...
        def is_swig_candidate():
            return (
                ...
                and target_module_partname == "_" + source_module.identifier.rpartition(".")[2]
            )
        ...
        # import schemes (namely, SWIG).
            ...
            self.msgout(
                2, "Invalid relative import", level, target_module_partname, target_attr_names
            )
            result = []
            for sub in target_attr_names or "*":
                m = self.createNode(
                    InvalidRelativeImport, "." * level + target_module_partname, sub
                )
            ...
            if target_modules is None:
                ...
                target_module = self.createNode(
                    MissingModule, _path_from_importerror(msg, target_module_partname)
                )
                self._updateReference(source_module, target_module, edge_data=edge_attr)
        ...
        assert len(target_modules) == 1, (
            "Expected import_hook() to" "return only one module but received: {}".format(
                target_modules
            )
        )
        ...
        if (
            isinstance(target_module, MissingModule)
            and is_swig_import is None
            and is_swig_candidate()
                    ...
                    try:
                        ...
                        if target_submodule is None:
                            ...
                            assert target_module.is_global_attr(
                                ...
                                target_submodule_partname, target_module.identifier
                            )
                        ...
                        if is_swig_import:
                            ...
                            if self.find_node(target_submodule_partname):
                                self.msg(
                                    ...
                                    "already exists"
                                    % (target_submodule_name, target_submodule_partname),
                                )
                            else:
                                self.msg(
                                    4,
        ...
        for inst in util.iterate_instructions(module_code_object):
            ...
            if inst.opname == "IMPORT_NAME":
                ...
                module._deferred_imports.append(
                    (have_star, (target_module_partname, module, fromlist, level), {})
                )
    ...
    def itergraphreport(self, name="G", flatpackages=()):
        ...
        for node, data, outgoing, incoming in nodes:
            ...
            yield '\t"%s" [%s];\n' % (
                node,
                ",".join(
                    [(cpatt % item) for item in nodevisitor(node, data, outgoing, incoming).items()]
                ),
            )
