...
LOADER_CACHE_DEST_PATH = (
    "lib/gdk-pixbuf"  # NOTE: some search & replace code depends on / being used on all platforms.
)
...
def _generate_loader_cache(gdk_pixbuf_query_loaders, libdir, loader_libs):
    ...
    cachedata = subprocess.run(
        [gdk_pixbuf_query_loaders, *loader_libs],
        check=True,
    ...
    msys2_prefix = '"' + os.path.abspath(
        os.path.join(libdir, "..", "lib", "gdk-pixbuf-2.0", "2.10.0")
    )
...
def hook(hook_api):
    ...
    if not gdk_pixbuf_query_loaders:
        logger.warning(
            "gdk-pixbuf-query-loaders executable not found in GI library directory or in PATH!"
        )
    else:
