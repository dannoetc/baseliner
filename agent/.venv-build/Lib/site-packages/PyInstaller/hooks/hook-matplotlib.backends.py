...
def _autodetect_used_backends(hook_api):
    ...
    for name, co in mpl_code_objs.items():
        ...
        if co_backends:
            logger.info(
                "Discovered Matplotlib backend(s) via `matplotlib.use()` call in module %r: %r",
                name,
                co_backends,
            )
    ...
    logger.info(
        "Trying determine the default backend as first importable candidate from the list: %r",
        candidates,
    )
...
def hook(hook_api):
    ...
    if backends_method == "auto":
        ...
        if isinstance(backends_method, str):
            ...
            assert isinstance(
                backends_method, list
            ), "User-provided backend name(s) must be either a string or a list!"
            backend_names = backends_method
    ...
    module_names = [
        _backend_module_name(backend) for backend in backend_names
    ]  # backend name -> module name
