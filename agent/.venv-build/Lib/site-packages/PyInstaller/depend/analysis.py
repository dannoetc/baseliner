...
class PyiModuleGraph(ModuleGraph):
    ...
    def _merge_rthooks(self, rthooks, uhd, uhd_path):
        ...
        for module_name, python_file_name_list in rthooks.items():
            ...
            assert isinstance(module_name, str), (
                "%s must be a dict whose keys are strings; %s is not a string."
    ...
    def process_post_graph_hooks(self, analysis):
        ...
        while True:
            ...
            for module_name, module_hook in self._hooks.items():
                ...
                self._bindepend_symlink_suppression.update(
                    module_hook.bindepend_symlink_suppression
                )
            ...
            if excluded_imports:
                ...
                def _exclude_module(module_name, excluded_imports, referrer_name):
                    ...
                    for excluded_import in excluded_imports:
                        ...
                        match = (
                            module_name_parts[: len(excluded_import_parts)] == excluded_import_parts
                        )
                        if match:
                            ...
                            referrer_match = (
                                referrer_name_parts[: len(excluded_import_parts)]
    ...
    def _safe_import_module(self, module_basename, module_name, parent_package):
        ...
        if hook is not None:
            ...
            logger.info(
                "Processing pre-safe-import-module hook %r from %r", hook_basename, hook_path
            )
            ...
            if not hasattr(hook_module, "pre_safe_import_module"):
                raise NameError(
                    "pre_safe_import_module() function not defined by hook %r." % hook_module
                )
    ...
    def _find_module_path(self, fullname, module_name, search_dirs):
        ...
        if hook is not None:
            ...
            if not hasattr(hook_module, "pre_find_module_path"):
                raise NameError(
                    "pre_find_module_path() function not defined by hook %r." % hook_module
                )
...
def get_bootstrap_modules():
    ...
    if is_win:
        loader_mods.append(
            ("pyimod04_pywin32", os.path.join(loaderpath, "pyimod04_pywin32.py"), "PYMODULE")
        )
