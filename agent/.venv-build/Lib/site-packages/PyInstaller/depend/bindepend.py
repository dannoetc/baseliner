...
def binary_dependency_analysis(binaries, search_paths=None, symlink_suppression_patterns=None):
    ...
    for dest_name, src_name, typecode in output_toc:
        ...
        for dep_name, dep_src_path in get_imports(src_name, search_paths):
            logger.debug(
                "Processing dependency, name: %r, resolved path: %r", dep_name, dep_src_path
            )
            ...
            dep_src_path = pathlib.Path(
                dep_src_path
            )  # Turn into pathlib.Path for subsequent processing
            ...
            dep_dest_path = _select_destination_directory(
                dep_src_path, parent_dir_preservation_paths
            )
            dep_dest_path = pathlib.PurePath(
                dep_dest_path
            )  # Might be a str() if it is just a basename...
            ...
            if compat.is_darwin and dep_dest_path.parent == pathlib.PurePath("."):
                if osxutils.is_framework_bundle_lib(dep_src_path):
                    ...
                    dep_dest_path = pathlib.PurePath(
                        dep_src_path.relative_to(framework_parent_path)
                    )
            ...
            if compat.is_win:
                ...
                logger.debug(
                    ...
                    "of symlink suppression path patterns.",
                    str(dep_dest_path),
                )
            else:
                logger.debug(
                    "Adding symbolic link from %r to top-level application directory.",
                    str(dep_dest_path),
                )
    ...
    existing_symlinks = set(
        [dest_name for dest_name, src_name, typecode in output_toc if typecode == "SYMLINK"]
    )
    ...
    for dependency_name, referring_binary in missing_dependencies:
        ...
        if dependency_toc_entry is None:
            ...
            if not dylib.warn_missing_lib(dependency_name):
                continue
            logger.warning(
                "Library not found: could not resolve %r, dependency of %r.",
                dependency_name,
                referring_binary,
            )
            ...
            if any(dependency_src_path.match(pattern) for pattern in symlink_suppression_patterns):
                logger.info(
                    ...
                    "due to source path matching one of symlink suppression path patterns.",
        ...
        if m:
            if compat.is_aix:
                ...
                if compat.is_linux:
                    ...
                    if re.fullmatch(
                        r"ld(64)?(-linux|-musl)?(-.+)?\.so(\..+)?", os.path.basename(lib)
                    ):
                        continue
...
def _get_imports_macholib(filename, search_paths):
            ...
            for command in header.commands:
                ...
                if cmd_type == LC_RPATH:
                    ...
                    if rpath.startswith("@") and not rpath.startswith(
                        ...
                        logger.warning(
                            "Unsupported rpath format %r found in binary %r - ignoring...",
                            rpath,
                            filename,
                        )
                        continue
    ...
    def _resolve_using_path(lib):
        ...
        if os.path.isabs(lib):
            if _dyld_shared_cache_contains_path is not None and _dyld_shared_cache_contains_path(
                lib
            ):
                return lib
...
def get_python_library_path():
    ...
    if not is_shared:
        ...
        py_suffix = (
            "t" if compat.is_nogil else ""
        )  # TODO: does Anaconda provide debug builds with "d" suffix?
        if compat.is_darwin:
            # macOS
