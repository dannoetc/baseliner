...
class PyiFrozenFinder:
    ...
    def __init__(self, path):
        ...
        for top_level_path in _TOP_LEVEL_DIRECTORY_PATHS:
            ...
            raise ImportError(
                "Failed to determine relative path w.r.t. top-level application directory."
            )
    ...
    def find_spec(self, fullname, target=None):
        ...
        if entry_data is None:
            ...
            if self.fallback_finder is not None:
                trace(
                    f"{self}: find_spec: attempting resolve using fallback finder {self.fallback_finder!r}."
                )
        ...
        typecode = entry_data[0]
        trace(
            f"{self}: find_spec: found {fullname!r} in PYZ as {pyz_entry_name!r}, typecode={typecode}"
        )
...
class PyiFrozenLoader:
    ...
    def __init__(self, name, pyz_archive, pyz_entry_name, is_package):
        ...
        if is_package:
            module_file = os.path.join(
                sys._MEIPASS, pyz_entry_name.replace(".", os.path.sep), "__init__.py"
            )
        else:
            module_file = os.path.join(
                sys._MEIPASS, pyz_entry_name.replace(".", os.path.sep) + ".py"
            )
    ...
    def exec_module(self, module):
        ...
        bytecode = self.get_code(
            spec.name
        )  # NOTE: get_code verifies that `spec.name` matches `self.name`!
        if bytecode is None:
...
def install():
    ...
    for idx, entry in enumerate(sys.path_hooks):
        ...
        trace(
            "PyInstaller: zipimporter hook not found in sys.path_hooks! Prepending our finder hook to the list."
        )
