...
def _retrieve_setuptools_info():
    ...
    if version >= (71, 0) and setuptools_vendor is not None:
        ...
        setuptools_vendor_paths = [
            pathlib.Path(path).resolve() for path in setuptools_vendor.__path__
        ]
        ...
        for candidate_name in VENDORED_TOP_LEVEL_NAMESPACE_CANDIDATES:
                ...
                is_vendored = [
                    any(
                        [
                            ...
                            or candidate_path == setuptools_vendor_path
            ...
            # Store paths
            vendored_namespace_package_paths[candidate_name] = [
                str(path) for path in candidate_path_attr
            ]
    ...
    if version >= (71, 0) and setuptools_vendor is not None:
        ...
        vendored_data += collect_data_files(
            "setuptools._vendor.jaraco.text", includes=["**/Lorem ipsum.txt"]
        )
...
class SetuptoolsInfo:
    ...
    def _load_setuptools_info(self):
        ...
        if setuptools_info is None:
            logger.debug(
                "%s: failed to obtain setuptools info: setuptools could not be imported.", self
            )
            return
    ...
    def _create_vendored_aliases(vendored_name, module_name, modules_list):
        ...
        return (
            (module_name + vendored_module[prefix_len:], vendored_module)
            for vendored_module in modules_list
...
def pre_safe_import_module_for_top_level_namespace_packages(api):
    ...
    if vendored == "fully":
        ...
        vendored_name = f"setuptools._vendor.{module_name}"
        logger.info(
            "Setuptools: %r appears to be a full setuptools-vendored copy - creating alias to %r!",
            module_name,
            vendored_name,
        )
...
def pre_safe_import_module(api):
    ...
    vendored_name = f"setuptools._vendor.{module_name}"
    logger.info(
        "Setuptools: %r appears to be a setuptools-vendored copy - creating alias to %r!",
        module_name,
        vendored_name,
    )
