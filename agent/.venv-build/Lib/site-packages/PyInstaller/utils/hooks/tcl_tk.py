...
def _get_tcl_tk_info():
    ...
    try:
        ...
        tkinter.TclError
    ):  # e.g. "Can't find a usable init.tcl in the following directories: ..."
        return None
...
class TclTkInfo:
    ...
    def _load_tcl_tk_info(self):
        ...
        if tcl_tk_info is None:
            logger.debug(
                "%s: failed to obtain Tcl/Tk info: tkinter/_tkinter could not be imported.", self
            )
            return
        ...
        if self.tkinter_extension_file:
            try:
                ...
                logger.warning(
                    "%s: failed to determine Tcl and Tk shared library location!",
                    self,
                    exc_info=True,
                )
            ...
            if compat.is_darwin:
                self.is_macos_system_framework = self._check_macos_system_framework(
                    self.tcl_shared_library
                )
        ...
        if "TK_LIBRARY" in os.environ:
            ...
            compat.is_darwin
            and self.tk_shared_library
            and (
                # is_framework_bundle_lib handles only fully-versioned framework library paths...
                (osxutils.is_framework_bundle_lib(self.tk_shared_library))
                or
            ...
            self.tk_data_dir = os.path.join(
                os.path.dirname(tk_lib_realpath), "Resources", "Scripts"
            )
        else:
            self.tk_data_dir = os.path.join(
        ...
        if self.is_macos_system_framework:
            logger.info(
                "%s: using macOS system Tcl/Tk framework - not collecting data files.", self
            )
        else:
            ...
            if os.path.isdir(self.tcl_data_dir):
                ...
                logger.warning(
                    "%s: Tcl library/data directory %r does not exist!", self, self.tcl_data_dir
                )
            ...
            if os.path.isdir(self.tk_data_dir):
                ...
                logger.warning(
                    "%s: Tk library/data directory %r does not exist!", self, self.tk_data_dir
                )
            ...
            if os.path.isdir(self.tcl_module_dir):
                ...
                logger.warning(
                    "%s: Tcl module directory %r does not exist!", self, self.tcl_module_dir
                )
