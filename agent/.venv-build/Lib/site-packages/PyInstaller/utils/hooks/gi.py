...
class GiModuleInfo:
    def __init__(self, module, version, hook_api=None):
        ...
        def _get_module_info(module, version):
            ...
            if new_api:
                ...
                try:
                    repo.require(
                        module, version, GIRepository.RepositoryLoadFlags.IREPOSITORY_LOAD_FLAG_LAZY
                    )
                except ValueError:
                    return None  # Module not available
    ...
    def get_libdir(self):
        ...
        raise ValueError(
            f"Could not resolve any shared library of {self.name} {self.version}: {self.sharedlibs}!"
        )
    ...
    def collect_typelib_data(self):
        ...
        hiddenimports += collect_submodules(
            "gi.overrides", lambda name: name.endswith("." + self.name)
        )
...
def compile_glib_schema_files(datas_toc, workdir, collect_source_files=False):
    ...
    if not schema_compiler_exe:
        logger.warning(
            "GLib schema compiler (glib-compile-schemas) not found! Skipping GLib schema recompilation..."
        )
        return datas_toc
    ...
    try:
        ...
        # minimize damage.
        logger.warning(
            "Failed to recompile GLib schemas! Returning collected files as-is!", exc_info=True
        )
    ...
    if old_compiled_file_hash is not None:
        ...
        if new_compiled_file_hash == old_compiled_file_hash:
            os.utime(
                compiled_file,
                ns=(old_compiled_file_stat.st_atime_ns, old_compiled_file_stat.st_mtime_ns),
            )
