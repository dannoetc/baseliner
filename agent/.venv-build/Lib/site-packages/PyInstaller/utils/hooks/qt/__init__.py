...
class QtLibraryInfo:
    ...
    def _load_qt_info(self):
        ...
        def _read_qt_library_info(package):
            ...
            if hasattr(QLibraryInfo, "path"):
                ...
                location = {
                    x: QLibraryInfo.path(getattr(QLibraryInfo.LibraryPath, x)) for x in path_names
                }
            else:
        ...
        if qt_info is None:
            logger.debug(
                "%s: failed to obtain Qt library info: %s.QtCore could not be imported.",
                self,
                self.namespace,
            )
            return
        ...
        self.qt_inside_package = (
            ...
            == resolved_qt_prefix_path  # PySide2 and PySide6 Windows PyPI wheels
    ...
    def collect_module(self, module_name):
        ...
        while imported_libraries:
            imported_lib_name, imported_lib_path = (
                imported_libraries.pop()
            )  # (name, fullpath) tuple
            ...
            if lib_name in self.shared_libraries:
                ...
                if qt_module_info.module:
                    if qt_module_info.module == short_module_name:
                        ...
                        imported_libraries.update(
                            bindepend.get_imports(imported_lib_path, qtlib_search_paths)
                        )
                    else:
        ...
        for translation_base_name in translation_base_names:
            ...
            if translation_files:
                datas += [
                    (translation_file, translation_dst) for translation_file in translation_files
                ]
            else:
                logger.warning(
    ...
    def collect_plugins(self, plugin_type):
        ...
        logger.debug(
            "%s: found plugin files for plugin type %r: %r", self, plugin_type, plugin_files
        )
        ...
        for plugin_file in plugin_files:
            ...
            if valid:
                ...
                logger.debug(
                    "%s: excluding plugin %r (%r)! Reason: %s",
                    self,
                    plugin_file,
                    plugin_type,
                    reason,
    ...
    def collect_extra_binaries(self):
        ...
        binaries += self._collect_all_or_none(
            ["libEGL.dll", "libGLESv2.dll"], ["d3dcompiler_??.dll"]
        )
    ...
    def collect_qtnetwork_files(self):
        ...
        if compat.is_win:
            ...
            logger.warning(
                "%s: QtNetwork: collection of OpenSSL not implemented for this platform!"
            )
            return []
    ...
    def _collect_qtnetwork_openssl_macos(self, openssl_version):
        ...
        for dylib in dylib_names:
            ...
            dylib_path = bindepend._resolve_library_path_in_search_paths(
                dylib, search_paths=[base_prefix_lib_dir]
            )
            if dylib_path is None:
                dylib_path = bindepend.resolve_library_path(
                    dylib, search_paths=[base_prefix_lib_dir, self.qt_lib_dir]
                )
            if dylib_path is None:
                continue
    ...
    def _collect_qtnetwork_openssl_linux(self, openssl_version):
        ...
        if openssl_version >= 0x10000000 and openssl_version < 0x10100000:
            ...
            logger.debug(
                "%s: QtNetwork: looking for OpenSSL 1.0.x shared libraries: %r", self, shlib_names
            )
        elif openssl_version >= 0x10100000 and openssl_version < 0x30000000:
            # OpenSSL 1.1.x
            shlib_names = (
            ...
            logger.debug(
                "%s: QtNetwork: looking for OpenSSL 1.1.x shared libraries: %r", self, shlib_names
            )
        elif openssl_version >= 0x30000000 and openssl_version < 0x40000000:
            # OpenSSL 3.0.x
            shlib_names = (
            ...
            logger.debug(
                "%s: QtNetwork: looking for OpenSSL 3.0.x shared libraries: %r", self, shlib_names
            )
        else:
    ...
    def collect_qtqml_files(self):
        ...
        if not qml_src_dir or not os.path.isdir(qml_src_dir):
            logger.warning(
                "%s: QML directory %r does not exist. QML files not packaged.", self, qml_src_dir
            )
            return [], []
        ...
        for qmldir_file in sorted(qmldir_files):
            ...
            try:
                ...
                binaries += [
                    (str(src_file), str(_compute_dest_dir(src_file)))
                    for src_file in plugin_binaries
                ]
                datas += [
                    (str(src_file), str(_compute_dest_dir(src_file))) for src_file in plugin_datas
                ]
            except Exception:
                logger.warning(
                    "%s: failed to process QML plugin directory %s", self, plugin_dir, exc_info=True
                )
    ...
    _qml_plugin_def = re.compile(
        r"^(?:(?:optional)\s+)?(?:plugin)\s+(?P<name>\w+)(?:\s+(?P<path>\.+))?$"
    )
    ...
    def _process_qml_plugin(self, qmldir_file):
        ...
        for plugin_binary in plugin_binaries:
            ...
            if not valid:
                logger.debug(
                    "%s: excluding QML plugin binary %r! Reason: %s",
                    self,
                    str(plugin_binary),
                    reason,
                )
        ...
        if invalid_binaries:
            logger.debug(
                "%s: excluding QML plugin directory %r due to invalid plugin binaries!",
                self,
                str(plugin_dir),
            )
            return [], []
    ...
    def collect_qtwebengine_files(self):
        ...
        if is_macos_framework:
            ...
            src_framework_path = os.path.join(
                self.location["LibrariesPath"], "QtWebEngineCore.framework"
            )
            ...
            bundled_qt_libs = (
                pathlib.Path(self.package_location) in pathlib.Path(src_framework_path).parents
            )
            if bundled_qt_libs:
            ...
            if len(versions) == 0:
                ...
                logger.warning(
                    "Found multiple versions in QtWebEngineCore.framework (%r) - using the last one!",
                    versions,
                )
            version = versions[-1]
            ...
            datas.append(
                (
                    os.path.join(self.location["DataPath"], resources),
            ...
            rel_helper_path = os.path.relpath(
                self.location["LibraryExecutablesPath"], self.location["PrefixPath"]
            )
            ...
            binaries.append(
                (os.path.join(self.location["LibraryExecutablesPath"], "QtWebEngineProcess*"), dest)
            )
            ...
            if not os.path.isfile(qt_conf_file):
                ...
                if rel_helper_path == ".":
                    ...
                    rel_prefix = os.path.join(
                        *[".." for _ in range(len(rel_helper_path.split(os.pathsep)))]
                    )
                ...
                if rel_prefix not in (".", ".."):
                    logger.warning(
                        "%s: unexpected relative Qt prefix path for QtWebEngineProcess qt.conf: %s",
                        self,
                        rel_prefix,
                    )
        ...
        if compat.is_linux:
            ...
            logger.debug(
                "%s: QtWebEngineCore is linked against libnss3.so; collecting NSS libraries...",
                self,
            )
...
def exclude_extraneous_qt_bindings(hook_name, qt_bindings_order=None):
    _QT_BINDINGS = [
        ...
        "PyQt6",
        "PySide6",
    ]  # Known bindings, and also their preferred order
    _QT_API_ENV = "QT_API"
    ...
    if len(available_qt_bindings) == 1:
        logger.info(
            "%s: selected %r as the only available Qt bindings.", hook_name, selected_qt_bindings
        )
    else:
