...
from PyInstaller import HOMEPATH, compat, isolated
...
def get_module_attribute(module_name: str, attr_name: str):
    ...
    try:
        ...
        raise AttributeError(
            f"Failed to retrieve attribute {attr_name} from module {module_name}"
        ) from e
...
def get_module_file_attribute(package: str):
    ...
    try:
        ...
        raise ImportError(
            f"Failed to obtain the __file__ attribute of package/module {package}!"
        ) from e
...
def is_module_satisfies(
    ...
    if version is not None:
        raise ValueError(
            "Calling is_module_satisfies with version argument is not supported anymore."
        )
    if version_attr is not None:
        raise ValueError(
            "Calling is_module_satisfies with version argument_attr is not supported anymore."
        )
...
def get_package_paths(package: str):
    ...
    if len(pkg_paths) > 1:
        logger.warning(
            "get_package_paths - package %s has multiple paths (%r); returning only first one!",
            package,
            pkg_paths,
        )
...
def collect_submodules(
    ...
    with isolated.Python() as isolated_python:
        while todo:
            ...
            name = todo.pop()
            modules, subpackages, on_error = isolated_python.call(
                _collect_submodules, name, on_error
            )
...
def _collect_submodules(name, on_error):
    ...
    try:
        ...
        if on_error in ("warn", "warn once"):
            ...
            logger.warning(
                f"Failed to collect submodules for '{name}' because importing '{name}' raised: {ex}"
            )
...
def collect_dynamic_libs(
    package: str, destdir: str | None = None, search_patterns: list = PY_DYLIB_PATTERNS
):
    """
    ...
    if not is_package(package):
        logger.warning(
            "collect_dynamic_libs - skipping library collection for module '%s' as it is not a package.",
            package,
        )
        return []
...
def collect_data_files(
    ...
    if not is_package(package):
        logger.warning(
            "collect_data_files - skipping data collection for module '%s' as it is not a package.",
            package,
        )
        return []
...
def collect_system_data_files(
    path: str, destdir: str | os.PathLike | None = None, include_py_files: bool = False
):
    """
...
def collect_all(
    ...
    datas = collect_data_files(
        package_name, include_py_files, excludes=exclude_datas, includes=include_datas
    )
...
def collect_delvewheel_libs_directory(package_name, libdir_name=None, datas=None, binaries=None):
    ...
    binaries += [
        (str(dll_file), str(dll_file.parent.relative_to(pkg_base)))
        for dll_file in libs_dir.glob("*.dll")
    ]
