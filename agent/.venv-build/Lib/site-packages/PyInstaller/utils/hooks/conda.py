...
class Distribution:
    ...
    def from_package_name(cls, name: str):
        ...
        raise ModuleNotFoundError(
            "Package {} is either not installed or was not installed using Conda.".format(name)
        )
...
def files(name: str, dependencies: bool = False, excludes: list | None = None) -> List[PackagePath]:
    ...
    """
    return [
        file for dist in _iter_distributions(name, dependencies, excludes) for file in dist.files
    ]
