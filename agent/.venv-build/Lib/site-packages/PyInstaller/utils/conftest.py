    ...
    def test_script(
        ...
        **kwargs,
        ...
        self._test_executables(
            app_name, args=app_args, runtime=runtime, run_from_path=run_from_path, **kwargs
        )
    ...
    def _run_executable_(self, args, exe_path, prog_env, prog_cwd, runtime):
        ...
        try:
            ...
            self._display_message(
                "RUN-EXE",
                f"Process exited on its own after {elapsed:.1f} seconds with return code {retcode}.",
            )
        except (
            (subprocess.TimeoutExpired)
            if psutil is None
            else (psutil.TimeoutExpired, subprocess.TimeoutExpired)
        ):
            if runtime:
                # When 'runtime' is set, the expired timeout is a good sign that the executable was running successfully
                # for the specified time.
                self._display_message(
                    "RUN-EXE", f"Process reached expected run-time of {runtime} seconds."
                )
                retcode = 0
            else:
                # Executable is still running and it is not interactive. Clean up the process tree, and fail the test.
                self._display_message(
                    "RUN-EXE", f"Timeout while running executable (timeout: {timeout} seconds)!"
                )
                retcode = 1
            ...
            if psutil is None:
                ...
                try:
                    self._display_message(
                        "RUN-EXE", "Stopping the process using Popen.terminate()..."
                    )
                    process.terminate()
                    ...
                    try:
                        self._display_message(
                            "RUN-EXE", "Stopping the process using Popen.kill()..."
                        )
                        process.kill()
                        ...
                        self._display_message(
                            "RUN-EXE", "Failed to stop the process (or its child process(es))!"
                        )
            else:
                ...
                for child_process in list(process.children(recursive=True)):
                    with contextlib.suppress(psutil.NoSuchProcess):
                        self._display_message(
                            "RUN-EXE", f"Stopping child process {child_process.pid}..."
                        )
                        child_process.kill()
                ...
                try:
                    self._display_message(
                        "RUN-EXE", f"Waiting for main process ({process.pid}) to stop..."
                    )
                    stdout, stderr = process.communicate(timeout=5)
...
def pyi_windowed_builder(pyi_builder: AppBuilder):
    ...
    def _run_executable_(args, exe_path, prog_env, prog_cwd, runtime):
        return subprocess.run(
            [exe_path, *args], env=prog_env, cwd=prog_cwd, timeout=runtime
        ).returncode
