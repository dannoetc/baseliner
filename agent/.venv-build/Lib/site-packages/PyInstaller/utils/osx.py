...
def _find_version_cmd(header):
    ...
    version_cmd = [
        cmd for cmd in header.commands if cmd[0].cmd in {LC_BUILD_VERSION, LC_VERSION_MIN_MACOSX}
    ]
    assert (
        len(version_cmd) == 1
...
def convert_binary_to_thin_arch(filename, thin_arch, output_filename=None):
    ...
    if p.returncode:
        raise SystemError(
            f"lipo command ({cmd_args}) failed with error code {p.returncode}!\noutput: {p.stdout}"
        )
...
def binary_to_target_arch(filename, target_arch, display_name=None):
    ...
    if target_arch == "universal2":
        ...
        if is_fat:
            ...
            logger.debug(
                "Converting fat binary %s (%s) to thin binary (%s)",
                filename,
                display_name,
                target_arch,
            )
...
def collect_files_from_framework_bundles(collected_files):
    ...
    for dest_name, src_name, typecode in collected_files:
        ...
        dest_path = pathlib.PurePath(
            dest_name
        )  # /dest/path/to/<name>.framework/Versions/<version>/<name>
        ...
        framework_files.add(
            (str(dest_path.parent.parent / "Current"), str(dest_path.parent.name), "SYMLINK")
        )
