...
class VSVersionInfo:
    ...
    def toRaw(self):
        ...
        return (
            struct.pack("HHH", sublen, vallen, typ)
            + raw_name
            + b"\000\000"
            + pad
            + rawffi
...
class StringTable:
    ...
    def fromRaw(self, data, i, limit):
        i, (cpsublen, cpwValueLength, cpwType, self.name) = parseCodePage(
            data, i, limit
        )  # should be code page junk
        i = nextDWord(i)
        while i < limit:
            ss = StringStruct()
...
class VarFileInfo:
    ...
    def toRaw(self):
        ...
        return (
            struct.pack("HHH", self.sublen, self.vallen, self.wType)
            + raw_name
            + b"\000\000"
            + pad
            + tmp
...
class VarStruct:
    ...
    def toRaw(self):
        ...
        return (
            struct.pack("HHH", self.sublen, self.wValueLength, self.wType)
            + raw_name
            + b"\000\000"
            + pad
            + tmp
...
def load_version_info_from_text_file(filename):
    ...
    try:
        ...
        raise ValueError(
            "Failed to deserialize VSVersionInfo from text-based representation!"
        ) from e
