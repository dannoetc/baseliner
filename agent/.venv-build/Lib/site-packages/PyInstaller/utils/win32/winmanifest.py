...
def _find_elements_by_tag(root, tag):
    ...
    return [
        ...
        if node.nodeType == xml.dom.Node.ELEMENT_NODE and node.tagName == tag
    ]
...
def _set_execution_level(manifest_dom, root_element, uac_admin=False, uac_uiaccess=False):
    ...
    requested_execution_level_element = _find_element_by_tag(
        requested_privileges_element, "requestedExecutionLevel"
    )
    ...
    requested_execution_level_element.setAttribute(
        "level", "requireAdministrator" if uac_admin else "asInvoker"
    )
...
def _ensure_common_controls_dependency(manifest_dom, root_element):
    ...
    for dependency_element in dependency_elements:
        ...
        # <assemblyIdentity>
        assembly_identity_element = _find_element_by_tag(
            dependent_assembly_element, "assemblyIdentity"
        )
        # Check the name attribute
        if (
            assembly_identity_element.attributes["name"].value
...
def read_manifest_from_executable(filename):
    ...
    if CREATEPROCESS_MANIFEST_RESOURCE_ID not in resources:
        raise ValueError(
            f"No RT_MANIFEST resource named CREATEPROCESS_MANIFEST_RESOURCE_ID found in {filename!r}."
        )
