...
class DeflateDecoder(ContentDecoder):
    ...
    @property
    def has_unconsumed_tail(self) -> bool:
        return bool(self._unfed_data) or (bool(self._obj.unconsumed_tail) and not self._first_try)
...
class GzipDecoder(ContentDecoder):
    ...
    def decompress(self, data: bytes, max_length: int = -1) -> bytes:
        ...
        while True:
            try:
                ret += self._obj.decompress(data, max_length=max(max_length - len(ret), 0))
            except zlib.error:
            ...
            self._unconsumed_tail = data = self._obj.unconsumed_tail or self._obj.unused_data
...
try:
    ...
    class ZstdDecoder(ContentDecoder):
        ...
        def decompress(self, data: bytes, max_length: int = -1) -> bytes:
            ...
            while (
                self._obj.eof and self._obj.unused_data and (max_length < 0 or length < max_length)
            ):
        ...
        @property
        def has_unconsumed_tail(self) -> bool:
            return not (self._obj.needs_input or self._obj.eof) or bool(self._obj.unused_data)
...
class HTTPResponse(BaseHTTPResponse):
    ...
    def read1(
        ...
        if self._has_decoded_content:
            ...
            if (
                ...
                decoded_data = self._decode(
                    ...
                    decode_content,
                    flush_decoder=False,
                    max_length=(amt - len(self._decoded_buffer) if amt is not None else None),
                )
                self._decoded_buffer.put(decoded_data)
        ...
        while True:
            ...
            decoded_data = self._decode(data, decode_content, flush_decoder, max_length=amt)
            self._decoded_buffer.put(decoded_data)
